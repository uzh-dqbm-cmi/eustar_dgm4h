...
from ..trainers.base_trainer.base_trainer import CustomTrainer
from ..trainers.base_trainer.rnn_trainer import RNNTrainer
...
class TrainingPipeline(Pipeline):
    ...
    def _check_dataset(self, dataset: BaseDataset):
        ...
        if "data_x" not in dataset_output.keys():
            raise DatasetError(
                "The Dataset should output a dictionnary with keys ['data_x']"
            )
        ...
        if isinstance(self.training_config, BaseTrainerConfig):
            if self.training_config.customized:
                if self.training_config.rnn:
                    ...
                    logger.info("Using customized Base Trainer\n")
