...
class TrainingCallback:
    ...
    def on_log_rnn(self, training_config: BaseTrainerConfig, logs, **kwargs):
        """
...
class CallbackHandler:
    ...
    def on_log_rnn(self, training_config: BaseTrainerConfig, logs, **kwargs):
        self.call_event("on_log_rnn", training_config, logs=logs, **kwargs)
...
class MetricConsolePrinterCallback(TrainingCallback):
    ...
    def on_log(self, training_config: BaseTrainerConfig, logs, **kwargs):
        ...
        if logger is not None:
            ...
            epoch_train_loss_cv = logs.get("train_epoch_loss_cv", None)
            epoch_eval_loss = logs.get("eval_epoch_loss", None)
            epoch_eval_loss_cv = logs.get("eval_epoch_loss_cv", None)

            epoch_train_loss_ce = logs.get("train_epoch_loss_ce", None)
            epoch_eval_loss_ce = logs.get("eval_epoch_loss_ce", None)
            epoch_train_loss_nll = logs.get("train_epoch_loss_nll", None)
            epoch_eval_loss_nll = logs.get("eval_epoch_loss_nll", None)

            epoch_train_loss_ce_pred = logs.get("train_epoch_loss_ce_pred", None)
            epoch_eval_loss_ce_pred = logs.get("eval_epoch_loss_ce_pred", None)
            epoch_train_loss_nll_pred = logs.get("train_epoch_loss_nll_pred", None)
            epoch_eval_loss_nll_pred = logs.get("eval_epoch_loss_nll_pred", None)

            epoch_train_losses = logs.get("train_epoch_losses", None)
            epoch_eval_losses = logs.get("eval_epoch_losses", None)
            epoch_train_losses_unw = logs.get("train_epoch_losses_unw", None)
            epoch_eval_losses_unw = logs.get("eval_epoch_losses_unw", None)

            epoch_train_losses_pred = logs.get("train_epoch_losses_pred", None)
            epoch_eval_losses_pred = logs.get("eval_epoch_losses_pred", None)
            epoch_train_losses_unw_pred = logs.get("train_epoch_losses_unw_pred", None)
            epoch_eval_losses_unw_pred = logs.get("eval_epoch_losses_unw_pred", None)
            ...
            if epoch_train_loss is not None:
                ...
                logger.info(f"Train loss CV: {np.round(epoch_train_loss_cv, 4)}")

                logger.info(f"Train losses: {np.round(epoch_train_losses, 4)}")
                logger.info(
                    f"Train loss CE all: {np.round(epoch_train_loss_ce, 4)} Train loss nll all: {np.round(epoch_train_loss_nll, 4)}"
                )
                logger.info(f"Train losses unw: {np.round(epoch_train_losses_unw, 4)}")

                logger.info(
                    f"Train losses pred: {np.round(epoch_train_losses_pred, 4)}"
                )
                logger.info(
                    f"Train loss CE recon pred: {np.round(epoch_train_loss_ce_pred, 4)} Train loss nll recon pred: {np.round(epoch_train_loss_nll_pred, 4)}"
                )

                logger.info(
                    f"Train losses unw pred: {np.round(epoch_train_losses_unw_pred, 4)}"
                )

            if epoch_eval_loss is not None:
                logger.info(f"Eval loss: {np.round(epoch_eval_loss, 4)}")
                logger.info(f"Eval loss CV: {np.round(epoch_eval_loss_cv, 4)}")
                logger.info(f"Eval losses: {np.round(epoch_eval_losses, 4)}")
                logger.info(
                    f"Eval loss CE recon: {np.round(epoch_eval_loss_ce, 4)} Eval loss nll recon: {np.round(epoch_eval_loss_nll, 4)}"
                )
                logger.info(f"Eval losses unw: {np.round(epoch_eval_losses_unw, 4)}")
                logger.info(f"Eval losses pred: {np.round(epoch_eval_losses_pred, 4)}")
                logger.info(
                    f"Eval loss CE recon pred: {np.round(epoch_eval_loss_ce_pred, 4)} Eval loss nll pred: {np.round(epoch_eval_loss_nll_pred, 4)}"
                )

                logger.info(
                    f"Eval losses unw pred: {np.round(epoch_eval_losses_unw_pred, 4)}"
                )
    ...
    def on_log_rnn(self, training_config: BaseTrainerConfig, logs, **kwargs):
        ...
        if logger is not None:
            epoch_train_loss = logs.get("train_epoch_loss", None)
            epoch_eval_loss = logs.get("eval_epoch_loss", None)
            epoch_train_loss_x = logs.get("train_epoch_loss_x", None)
            epoch_train_loss_y = logs.get("train_epoch_loss_y", None)
            epoch_train_loss_ce = logs.get("train_epoch_loss_ce", None)
            epoch_train_loss_nll = logs.get("train_epoch_loss_nll", None)
            epoch_eval_loss_x = logs.get("eval_epoch_loss_x", None)
            epoch_eval_loss_y = logs.get("eval_epoch_loss_y", None)
            epoch_eval_loss_ce = logs.get("eval_epoch_loss_ce", None)
            epoch_eval_loss_nll = logs.get("eval_epoch_loss_nll", None)
            ...
            if epoch_train_loss is not None:
                logger.info(
                    f"Train loss all: {np.round(epoch_train_loss, 4)}, 'x': {np.round(epoch_train_loss_x, 4)}, 'y': {np.round(epoch_train_loss_y, 4)}"
                )
                logger.info(
                    f"Train ce: {np.round(epoch_train_loss_ce, 4)}, 'nll': {np.round(epoch_train_loss_nll, 4)}"
                )

            if epoch_eval_loss is not None:
                logger.info(
                    f"Eval loss all: {np.round(epoch_eval_loss, 4)}, 'x': {np.round(epoch_eval_loss_x, 4)}, 'y': {np.round(epoch_eval_loss_y, 4)}"
                )
                logger.info(
                    f"Eval ce: {np.round(epoch_eval_loss_ce, 4)}, 'nll': {np.round(epoch_eval_loss_nll, 4)}"
                )
