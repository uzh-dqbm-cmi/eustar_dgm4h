...
class Test_Build_Scheduler:
    ...
    def schedulers(self, request, optimizers):
        if request.param[0] is not None:
            ...
            discriminator_scheduler = request.param[0](
                optimizers[2], **request.param[1]
            )
...
class Test_Main_Training:
    ...
    def test_train_2_steps_updates(
        self, ae, train_dataset, training_configs, optimizers, optimizer_updates
    ):
        trainer = CoupledOptimizerAdversarialTrainer(
            model=ae,
            train_dataset=train_dataset,
